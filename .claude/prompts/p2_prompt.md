...
Điều kiện kích hoạt DUY NHẤT: user nói rõ P2 đã xong / đã duyệt. Bạn vừa báo cáo
...
1. Danh sách việc lấy bằng MỘT lệnh — dò theo tag phase, ĐỪNG dùng số dòng (số dòng
   trôi sau mỗi lần sửa plan):
       grep -nE '(class="c">|<td>)(⬜|⚠) (P[0-9] · )*P2( |<)' docs/reports/plan-v4.html
   Neo vào `class="c"`/`<td>` để KHÔNG bắt văn xuôi tiền đề trong tab 07.
   `grep '⬜ P2'` trần thì BỎ SÓT marker đa phase — ví dụ thật: dòng `extract/`
   mang `⬜ P1 · P3 · P5`.
...
3. Hai bảng trạng thái khác phải khớp theo:
     ...
     tab 04 bảng runbook 8 ô — cột "Trạng thái hôm nay" (pill nào P2 vừa mở khoá)
...
5. Tab 08 Checklist lưu ở localStorage của TRÌNH DUYỆT, không có gì trong file để sửa.
   BẢO USER tự tick P2-1…P2-6. ĐỪNG báo là đã tick hộ.
6. Commit RIÊNG, đừng trộn với code: `docs: mark P2 progress in plan-v4`.
```
