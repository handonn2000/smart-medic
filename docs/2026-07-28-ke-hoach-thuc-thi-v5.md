...
### 1.5 Hiện trạng file (nhánh `claude/smart-medic-research-dd4e21`, **chưa commit**)

```
bench/                              hạ tầng đo lường — stdlib thuần, 8 lệnh
data/dev_gold_consensus/            20 file gold (LLM đồng thuận)  ⚠ xem bẫy #3
data/gold_variants/{consensus,sonnet5,opus5,prefill}/   4 biến thể gold
data/runs/{v3,v4}/                  output thật của hai hệ, 100 file mỗi hệ
data/gen/{synthetic,translated,restyled}/               nhãn của dữ liệu sinh
data/generated_medical_records/*/text/                  văn bản của dữ liệu sinh
docs/reports/2026-07-28-huong-tiep-can-moi.html         báo cáo 10 tab
```

**Việc đầu tiên:** commit toàn bộ khối này trước khi thay đổi gì, để mọi số đo
sau có mốc so sánh.
...
### 2.5 Dữ liệu sinh sẵn có (314 tài liệu, 9.655 span, **0 lỗi offset**)

| tập | file | span | Δ độ dài span | nhãn thiếu | THUỐC rỗng |
|---|---:|---:|---:|---:|---:|
| dev_gold (chuẩn) | 20 | 689 | — | 15,2% *(nền)* | 76% |
| `gen/synthetic` | 194 | 5.353 | **−0,12** ✅ | 30,5% ❌ | 10% ❌ |
| `gen/translated` | 95 | 3.368 | **−1,11** ❌ | 9,0% ✅ | 9% ❌ |
| `gen/restyled` | 25 | 934 | **−1,20** ❌ | 6,1% ✅ | 9% ❌ |

- `synthetic` **dùng làm tập chấm chính** cho chất lượng span: MDE **±0,011** so
...
- 741 mention THUỐC có RxCUI (dev gold chỉ có **16**) — dùng cho GP-03.
